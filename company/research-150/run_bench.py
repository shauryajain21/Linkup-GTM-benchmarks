#!/usr/bin/env python3
"""
Company-research benchmark harness (v2) — agentic retrieval → synth → judge, with
per-stage persistence and concurrent judging.

Pipeline per (company, section, engine):
  Stage 1  RETRIEVAL  — each engine scrapes the company page + runs the section's searches
                        (agentic searchResults; no generated answer). -> out/raw.jsonl
  Stage 2  SYNTH      — Claude Sonnet formats that engine's raw results into structured JSON,
                        grounded only in the results, no truncation.  -> out/synth.jsonl
  Stage 3  JUDGE      — per (company, section), once all engines' synth is ready, Opus scores
                        right_company / entity_relevance / topical_relevance. -> out/judge.jsonl
                        (Quantity / completeness / source-mix are computed deterministically at
                         --aggregate time, not by the LLM.)

Each (company, section) group judges as soon as ITS engines finish — concurrent across groups.
All stages are resumable (skip keys already in the stores).

Disambiguation: every prompt is anchored with a rich block from the company DB
(domain, HQ, founded, funding, lead investors, Crunchbase URL) so engines target the
right entity, not a same-named namesake.

USAGE
  python3 run_company_bench.py --dry-run                 # print plan + rendered prompts, NO API calls
  python3 run_company_bench.py --limit 2                 # live smoke test on 2 companies
  python3 run_company_bench.py --limit 50 --concurrency 6
  python3 run_company_bench.py --aggregate               # build scorecard from the stores
"""
...
SYNTH_MAX, JUDGE_MAX = 16000, 4000   # synth high enough that no list truncates (effectively uncapped here)
csv.field_size_limit(10 ** 8)

# Per-provider QPS limits (requests/sec). Edit here if limits change.
# Anthropic covers BOTH synth + judge (they share this bucket).
RATES = {"linkup": 25.0, "exa": 10.0, "perplexity": 10.0, "parallel": 10.0, "anthropic": 10.0}
TIMINGS = []  # [label, ms] per successful HTTP round-trip (excludes rate-limiter wait)
...
def load_companies(limit=None, offset=0):
    ...
    out = [{k: (r.get(k) or "").strip() for k in cols} for r in csv.DictReader(open(DB))]
...
class Rate:
    """Async QPS limiter — paces request *starts* at `qps`, queuing concurrent callers."""
...
def _fmt_results(results):
    """Stringify a list of {title,url,text/snippet/highlights} into a readable blob + collect URLs."""
    lines, urls = [], []
...
async def retrieve(client, engine, sec, c, rate):
    """Run the section's searches (+ scrape the company page where supported) on one engine."""
    queries = render_searches(sec, c)
    scrape = SECTIONS[sec]["scrape"]
    ...
    if engine == "linkup":
        ...
        for q in queries:
            tasks.append(collect(f"search:{q[:30]}", _post(client, "https://api.exa.ai/search",
                         h, {"query": q, "type": "auto", "numResults": 10, "contents": {"text": True}}, rate=rate, label="retrieve:"+engine)))
        if scrape:
...
async def run(companies, concurrency):
    ...
    # Real throttle = per-provider QPS limiters. `concurrency` only bounds how many
    # (company, section) groups are in flight (to cap memory), not the request rate.
...
def _ikey(sec, it):
    """Normalized identity of one item, for dedup."""
    if not isinstance(it, dict):
...
def count_items(sec, structured):
    """Returns (raw_count, deduped_count) of the section's list."""
...
def aggregate():
    ...
    itemsets = defaultdict(lambda: defaultdict(set))  # (company,sec) -> engine -> {item keys}  (cust+case)
    relC = defaultdict(lambda: defaultdict(list))     # company -> engine -> [per-section relevance]
    for k, rec in synth_d.items():
        ...
        eng[engine]["qty"].append(ded_n)        # deduped is the headline quantity
        ...
        # source mix from raw urls
        for u in raw.get(k, {}).get("urls", []):
            eng[engine]["src"][classify(u, "")] += 1  # domain match handled below
    # source mix needs domain; recompute with company domain
    comp_dom = {}
    ...
    # unique recall + union coverage (cust+case entity sections)
    ...
    # consistency: P10 relevance + bomb rate (companies <40 avg relevance)
    def pctile(xs, pp):
    ...
    for e in ENGINES:
        ...
        card["engines"][e] = {
            "items_per_company_deduped": round(d["ded_tot"] / max(1, n_companies), 1),   # 5-section total per company
    ...
    # latency (per-call HTTP round-trip, excludes rate-limiter wait) from timings.jsonl
...
def dry_run(companies):
    ...
    for sec in SECTIONS:
        ...
        for q in render_searches(sec, c):
            print(f"   • {q}")
        print(f"   scrape: {SECTIONS[sec]['scrape']}  -> {c['url']}")
    print("\n─" * 1)
